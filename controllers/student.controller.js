...
exports.getStudentsWithBalances = async (req, res) => {
    try {
        // Fetch students with their balance details
        const students = await Student.findAll({
            attributes: ['first_name', 'last_name', 'class'],
            include: [
                {
                    model: StudentFinance,
                    attributes: ['school_fees_amount'],
                },
                {
        ...
        // Filter out students with outstanding balances
        const studentsWithBalances = students.map(student => {
            const totalFees = student.studentfinance ? student.studentfinance.school_fees_amount : 0;
            const totalPayments = student.studentpayments.reduce((sum, payment) => sum + payment.amount_paid, 0);
            const balanceDue = totalFees - totalPayments;
        ...
        }).filter(student => student.balance_due > 0);

        // Send response
        res.send({
            status: "Success",
            status_code: 1000,
            message: "Students with balances successfully retrieved",
            results: studentsWithBalances
        });
    } catch (err) {
        res.status(500).send({
            status: "Error",
            status_code: 1001,
            message: err.message || "Error occurred while retrieving students with balances"
        });
    }
};